        ...
        try {
            ...
            if (needBuild.nonEmpty) {
                ...
                needBuild.foreach { gid =>
                    // Match case-INSENSITIVELY: the HB version string is
                    // "Cthulhu Wars HRF Homebrew v X" (capital H), so the old
                    // line0.contains("homebrew") never matched and every HB
                    // game fell through to the else-branch and was mis-tagged
                    // "library" (2026-08-04 fix). Also: the fallback is now
                    // "" (unknown/unclassified) instead of "library" — a game
                    // we cannot positively identify must NOT be silently
                    // labelled library, which is exactly what hid this bug.
                    val line0 = q(logs.filter(l => l.gameId === gid && l.index === 0).map(_.value).result.headOption).getOrElse("").toLowerCase
                    val build = if (line0.contains("library-at-celaeno")) "library"
                        else if (line0.contains("more-neutral-units")) "mnu"
                        else if (line0.contains("tcho-tcho") || line0.contains("tchotcho")) "tt"
                ...
                parameter("bot".as[Boolean].?(false)) { botFlag =>
                    decodeRequest {
                        entity(as[String]) { body =>
                            ...
                            touchMeta(newGameId)
                            // Derive the build from the game's version line (log
                            // index 0) instead of hard-coding "library". Every
                            // non-Library game (HB/MNU/TT/BB) was previously
                            // stamped "library" here, which is what mis-filed
                            // them in the admin console (2026-08-04 fix). Match
                            // case-insensitively — the HB string is "Homebrew"
                            // (capital H). Fall back to "" (unknown) when we
                            // can't positively identify it, never "library".
                            val v0 = lgs.headOption.getOrElse("").toLowerCase
                            val newBuild =
                                if (v0.contains("library-at-celaeno")) "library"
                                else if (v0.contains("more-neutral-units")) "mnu"
                                else if (v0.contains("tcho-tcho") || v0.contains("tchotcho")) "tt"
                                else if (v0.contains("bubastis")) "bb"
                                else if (v0.contains("homebrew")) "hb"
                                else ""
                            try { q(adminAnnotations += AdminAnnotation(newGameId, false, "", newBuild)) } catch { case _ : Throwable => () }
                            if (botFlag)
